...
HEAVY_PATHS=(
  ...
  '^models/'                        'mongoose schemas — the persistence boundary'
  '^lib/dbConnect\.ts$'             'database connection'
  '^lib/validations/'               'input validation at the trust boundary'
  '^\.claude/(hooks|settings|agents)' 'the agent guards themselves'
  '^(eslint|tsconfig|next|postcss)\.' 'build or lint boundary config'
)
#
